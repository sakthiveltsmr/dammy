...
window.onmouseout=(event)=>{
   ...
   window.addEventListener("mousedown",()=>{
    const storageType = cookieStorage;
    const consentPropertyName = 'coupon';
    const shouldShowPopup = () => !storageType.getItem(consentPropertyName);
    const saveToStorage = () => storageType.setItem(consentPropertyName, true);
 ...
 Addtocart.addEventListener("click",()=>{
    const storageType = cookieStorage;
    const consentPropertyName = 'Addtocart';
    const shouldShowPopup = () => !storageType.getItem(consentPropertyName);
    const saveToStorage = () => storageType.setItem(consentPropertyName, true);
    const value=shouldShowPopup()
    if(value){
        ...
        let main= document.getElementById('root')
           ...
           const data=document.createElement('div')
           data.innerHTML=(`<div id="iapg" style="alignItem:center; top:0px; width:300px; height:100px; border:3px solid #00FF00; margin-left: 800px;
           position: absolute">
